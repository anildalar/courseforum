...
c,b={})=>{"string"===typeof c&&(c=XF.customEvent(c,b));const f=c.type,[d,e="default"]=f.split(".");if("default"===e)return a.dispatchEvent(c);if(!XF.eventHandlers[e]||!XF.eventHandlers[e][d])return!(c.cancelable&&c.defaultPrevented);XF.eventHandlers[e][d].forEach(g=>{if(a===g.element){const {handler:h,options:k}=g;k.once&&XF.off(a,f,h,k);h.call(a,c);k.passive&&c.defaultPrevented&&console.warn("preventDefault() was called on a namespaced passive event listener and this is not supported.")}});return!(c.cancelable&&
c.defaultPrevented)},onDelegated:(a,c,b,f,d={})=>{const e=g=>{const h=g.target.closest(b);h&&a.contains(h)&&f.call(h,g)};e.originalHandler=f;XF.on(a,c,e,d)},on:(a,c,b,f={})=>{a?c.split(" ").forEach(d=>{const [e,g="default"]=d.split(".");d={element:a,handler:b,options:f};XF.eventHandlers[g]||(XF.eventHandlers[g]={});XF.eventHandlers[g][e]||(XF.eventHandlers[g][e]=[]);XF.eventHandlers[g][e].push(d);a.addEventListener(e,b,f)}):console.error("No element passed in to event.")},off:(a,c,b=null,f=!1)=>{const [d,
...
null,l.onerror=null,d.appendChild(l),h()};for(const l of a)if(!XF.loadedScripts[l])if(XF.loadedScripts[l]=!0,f)XF.createElement("script",{src:l,async:!1,onload:h,onerror:h},d);else if(b){const p=XF.createElement("script",{onreadystatechange:k,onerror:()=>{p.onreadystatechange=null;p.onerror=null;h()},url:l});g.push(p)}else{const p=new XMLHttpRequest;p.open("GET",l,!0);p.onload=()=>{200<=p.status&&400>p.status&&XF.createElement("script",{text:p.response},d);h()};p.onerror=h;p.send()}!e&&c&&c()},async ajax(a,
c,b={},f,d={}){"function"===typeof b&&void 0===f&&(f=b,b={});b instanceof HTMLFormElement&&(b=new FormData(b));let e=!0,g=!0,h=!0,k=!0;d.skipDefault&&(g=e=!1,delete d.skipDefault);d.skipDefaultSuccessError&&(g=!1,delete d.skipDefaultSuccessError);d.skipDefaultSuccess&&(e=!1,delete d.skipDefaultSuccess);d.skipError&&(h=!1,delete d.skipError);void 0!==d.global&&(k=d.global?!0:!1,delete d.global);const l=(r,t,w)=>{XF.trigger(m,XF.customEvent("ajax:before-success",{request:r,response:t,status:t.status,
data:w}));g&&XF.defaultAjaxSuccessError(w,t.status,t,r)||e&&XF.defaultAjaxSuccess(w,t.status,t,r)||f&&f(w,t.status,t,r)},p=(r,t,w,z=null)=>{h&&XF.defaultAjaxError(w,null==t?void 0:t.status,t,r,z)},n={"X-Requested-With":"XMLHttpRequest"};let u;var x=d.dataType||"json";delete d.dataType;switch(x){case "html":var v=r=>r.text();n.Accept="text/html";break;case "json":v=r=>r.json();n.Accept="application/json";break;case "xml":v=r=>r.text();n.Accept="application/xml";break;default:throw Error(`Unsupported dataType: ${x}`);
}"json"!==x&&(e=!1);b=XF.dataPush(b,"_xfResponseType",x);b=XF.dataPush(b,"_xfWithData",1);b=XF.dataPush(b,"_xfRequestUri",q.location.pathname+q.location.search);XF.config.csrf&&(b=XF.dataPush(b,"_xfToken",XF.config.csrf));if("GET"===a.toUpperCase())c+=(c.includes("?")?"&":"?")+(new URLSearchParams(b)).toString();else if(b instanceof FormData)u=b;else if(Array.isArray(b))u=XF.Serializer.serializeFormData(b),n["Content-Type"]="application/x-www-form-urlencoded";else{function r(t){if(t instanceof Map)return Object.fromEntries([...t.entries()].map(([w,
z])=>[w,r(z)]));if(t instanceof Set)return[...t].map(r);if(Array.isArray(t))return t.map(r);if(t&&"object"===typeof t){const w={};for(const [z,A]of Object.entries(t))w[z]=r(A);return w}return t}u=JSON.stringify(r(b));n["Content-Type"]="application/json"}const y=new AbortController;x=y.signal;d.signal&&d.signal.addEventListener("abort",()=>{y.abort()});delete d.signal;b="GET"===a.toUpperCase()?3E4:6E4;b=setTimeout(()=>{y.abort(new DOMException("_XF_TIMEOUT","AbortError"))},d.timeout||b);delete d.timeout;
a=new Request(c,{method:a,headers:n,body:u,signal:x,...d});XF.trigger(m,XF.customEvent("ajax:send",{request:a}));k&&XF.trigger(m,"xf:action-start");try{const r=await fetch(a);let t;if(r.ok)t=await v(r),l(a,r,t);else{t=await r.text();v=!1;try{t=JSON.parse(t),v=!0}catch(w){}v?l(a,r,t):p(a,r,t)}XF.trigger(m,XF.customEvent("ajax:complete",{request:a,response:r,data:t}));return{request:a,response:r,data:t}}catch(r){return p(a,null,null,r),XF.trigger(m,XF.customEvent("ajax:complete",{request:a,error:r})),
{request:a,error:r}}finally{clearTimeout(b),k&&XF.trigger(m,"xf:action-stop")}},ajaxAbortable(a,c,b={},f,d={}){const e=new AbortController;d.signal=e.signal;return{ajax:XF.ajax(a,c,b,f,d),controller:e}},dataPush:(a,c,b)=>{a&&"string"!==typeof a?void 0!==a[0]?a.push({name:c,value:b}):a instanceof FormData?a.append(c,b):a[c]=b:(a=String(a),a+="&"+encodeURIComponent(c)+"="+encodeURIComponent(b));return a},defaultAjaxSuccessError:(a,c,b,f)=>{if("object"!==typeof a)return XF.alert("Response was not JSON."),
!0;a.html&&a.html.templateErrors&&(c="Errors were triggered when rendering this template:",a.html.templateErrorDetails&&(c+="\n* "+a.html.templateErrorDetails.join("\n* ")),console.error(c));return a.errorHtml?(XF.setupHtmlInsert(a.errorHtml,(d,e)=>{e=e.h1||e.title||XF.phrase("oops_we_ran_into_some_problems");XF.overlayMessage(e,d)}),!0):a.errors?(XF.alert(a.errors),!0):a.exception?(XF.alert(a.exception),!0):!1},defaultAjaxSuccess:(a,c,b,f)=>{a&&"ok"===a.status&&a.message&&XF.flashMessage(a.message,
3E3);return!1},defaultAjaxError:(a,c,b,f,d)=>{if(d instanceof DOMException&&"AbortError"===d.name)"_XF_TIMEOUT"===d.message&&XF.alert(XF.phrase("server_did_not_respond_in_time_try_again"));else{if(d instanceof Error)throw d;console.error("Error:",a);XF.alert(XF.phrase("oops_we_ran_into_some_problems_more_details_console"))}},activate:a=>{XF.addExtraPhrases(a);XF.IgnoreWatcher.refresh(a);XF.Element.initialize(a);XF.DynamicDate.refresh(a);XF.ExpandableContent.checkSizing(a);XF.UnfurlLoader.activateContainer(a);
XF.KeyboardShortcuts.initializeElements(a);XF.FormInputValidation.initializeElements(a);XF.LazyHandlerLoader.loadLazyHandlers(a);if(q.FB){const c=a instanceof Document?a.body:a;setTimeout(()=>FB.XFBML.parse(c),0)}XF.trigger(m,XF.customEvent("xf:reinit",{element:a}))},activateAll:a=>{Array.from(a).forEach(c=>XF.activate(c))},getDefaultFormData:(a,c,b,f)=>{let d,e;c&&c.hasAttribute("name")&&(e=c.getAttribute("name"));b&&"multipart/form-data"===a.getAttribute("enctype")&&console.error("JSON serialized forms do not support the file upload-style enctype.");
if(q.FormData&&!b)d=new FormData(a),e&&d.append(e,c.getAttribute("value")||e),a.querySelectorAll('input[type="file"]').forEach(g=>{const h=g.files;if("undefined"!==typeof h&&0===h.length)try{d.delete(g.getAttribute("name"))}catch(k){}});else{if(b){a=a.elements;let g;const h=[],k=[];if(f){"string"===typeof f&&(f=f.split(","));const l=[];f.forEach((p,n)=>{"number"===typeof n?l.push(XF.regexQuote(p.trim())):l.push(XF.regexQuote(n.trim()))});l.length&&(g=new RegExp("^("+l.join("|")+")(\\[|$)"))}Array.from(a).forEach(l=>
{const p=l.name;p&&"_xf"!==p.substring(0,3)?!g||g.test(p)?h.push(l):k.push(l):k.push(l)});d=XF.Serializer.serializeArray(k);f=XF.Serializer.serializeJSON(h);d.unshift({name:b,value:JSON.stringify(f)})}else d=XF.Serializer.serializeArray(a);e&&d.push({name:e,value:e})}return d},scriptMatchRegex:/<script([^>]*)>([\s\S]*?)<\/script>/ig,setupHtmlInsert:(a,c,b)=>{"string"===typeof a&&(a={content:a});if("object"==typeof a&&a.content){if(b&&a.content&&!a.js){let f=a.content;a.js=[];let d;for(;d=/<script[^>]*src="([^"]+)"[^>]*><\/script>/i.exec(f);)a.js.push(d[1]),
f=f.substring(d.index+d[0].length)}XF.Loader.load(a.js,a.css,()=>{const f=a.jsInline||[];var d=a.content;var e="string"==typeof d;if(a.cssInline)for(var g of a.cssInline)XF.createElement("style",{textContent:g},m.head);if(e&&(d=d.trim(),!b))for(;e=XF.scriptMatchRegex.exec(d);){g=!1;const l=e[1].match(/(^|\s)type=("|'|)([^"' ;]+)/);if(l)switch(l[3].toLowerCase()){case "text/javascript":case "text/ecmascript":case "application/javascript":case "application/ecmascript":g=!0}else g=!0;g&&(f.push(e[2]),
d=d.replace(e[0],""))}let h=XF.createElementFromString(d);if(h){(h instanceof HTMLElement?h.querySelectorAll("noscript"):[]).forEach(l=>{l.parentNode.removeChild(l)});if(c instanceof HTMLElement){const l=c;c=p=>{l.innerHTML="";l.append(p);XF.activate(l)}}if("function"!==typeof c)console.error("onReady was not a function");else{var k=!1;d=(l,p)=>{if(!k){k=!0;for(const n of f)eval(n);a.jsState&&(XF.config.jsState=XF.applyJsState(XF.config.jsState,a.jsState));l||(p&&(h=p),h instanceof HTMLElement&&XF.activate(h))}};
!1!==c(h,a,d)&&d()}}})}else console.error("Was not provided an object or HTML content")},alert:(a,c,b,f)=>{let d=a;XF.isPlainObject(a)&&(d="<ul>",Object.keys(a).forEach(e=>{d+="<li>"+a[e]+"</li>"}),d+="</ul>",d='<div class="blockMessage">'+d+"</div>");c||(c="error");if(!b)switch(c){case "error":b=XF.phrase("oops_we_ran_into_some_problems");break;default:b=""}return XF.overlayMessage(b,d)},getOverlayHtml:a=>{var c={dismissible:!0,title:null,url:null};"object"===typeof a&&a.constructor===Object&&(c=
{...c,...a},a.html&&(a=a.html));if("string"==typeof a)a=XF.createElementFromString(a);else if(!(a instanceof HTMLElement))throw Error("Can only create an overlay with html provided as a string or DOM Element");if(!a.classList.contains("overlay")){var b=c.title;if(!b){var f=a.querySelector(".overlay-title");f&&(b=f.textContent,f.parentNode.removeChild(f))}b||(b=m.querySelector("title").textContent);(f=a.querySelector(".overlay-content"))&&(a=f);f=XF.createElement("div",{className:"overlay",tabIndex:"-1",
dataset:{url:c.url}});b=XF.createElement("div",{className:"overlay-title",innerHTML:b});c.dismissible&&(c=XF.createElement("a",{className:"overlay-titleCloser js-overlayClose",role:"button",tabIndex:0,ariaLabel:XF.phrase("close")}),b.prepend(c));c=XF.createElement("div",{className:"overlay-content"});XF.isCreatedContainer(a)?c.append(...a.childNodes):c.append(a);f.appendChild(b);f.appendChild(c);a=f}m.body.appendChild(a);return a},createMultiBar:(a,c,b,f)=>{},getMultiBarHtml:a=>{"object"===typeof a&&
a.constructor===Object&&a.html&&(a=a.html);if("string"==typeof a)a=XF.createElementFromString(a);else if(!(a instanceof HTMLElement))throw Error("Can only create an action bar with html provided as a string or DOM Element");const c=XF.createElement("div",{className:"multiBar",tabIndex:-1}),b=XF.createElement("div",{className:"multiBar-inner",innerHTML:"<span>Hello there.</span>"});b.appendChild(a);c.appendChild(b);m.body.appendChild(c);return c},overlayMessage:(a,c)=>{if("string"==typeof c)c=XF.createElementFromString(c),
c instanceof HTMLElement||(c=XF.createElement("div",{innerText:c.textContent}));else if(!(c instanceof HTMLElement))throw Error("Can only create an overlay with html provided as a string or DOM Element");if(!c.matches(".block, .blockMessage")&&!c.querySelector(".block, .blockMessage")){const b=XF.createElement("div",{className:"blockMessage"});b.appendChild(c);c=b}c=XF.getOverlayHtml({title:a,html:c});return XF.showOverlay(c,{role:"alertdialog"})},flashMessage:(a,c,b)=>{a=XF.createElementFromString('<div class="flashMessage-content">'+
...
c)=>{let b=864E5;switch(c){case "year":b*=365;break;case "month":b*=30}b*=a;return new Date(Date.now()+b)},smoothScroll:(a,c,b,f)=>{let d;a instanceof HTMLElement||"string"===typeof a?((a=a instanceof HTMLElement?a:m.querySelector(a))?(d=a.getBoundingClientRect().top+q.scrollY,b=parseInt(getComputedStyle(m.documentElement).getPropertyValue("scroll-padding-top"),10),isNaN(b)||(d-=b)):d=null,!0===c&&(c=a?"#"+a.id:null)):"number"===typeof a&&(d=a);if(null===d)console.error("Invalid scroll position");
...
delta:c.swing,complete:()=>{},start:()=>{},finish:()=>{},...d};a||(d.speed=0);if(f instanceof Element){var e=d.start(f);if(!1===e)d.complete();else{void 0===e&&(e={});var g=new Date,h=()=>{var k=new Date-g;k=0<d.speed?Math.min(1,k/d.speed):1;const l=d.delta(k);e={...e,...d.step(f,{...e,delta:l})};1===k?(d.finish(f,e),setTimeout(d.complete,0)):XF.requestAnimationTimeout(h)};return XF.requestAnimationTimeout(h)}}else d.complete()};return{enable:()=>{a=!0},disable:()=>{a=!1},easing:c,animate:b,fadeOut:(f,
d={})=>b(f,{...d,start:e=>{if("none"===q.getComputedStyle(e).display)return!1},step:(e,{delta:g})=>{e.style.opacity=(1-g).toString()},finish:e=>{XF.display(e,"none");e.style.opacity=""}}),fadeIn:(f,d={})=>b(f,{...d,start:e=>{if("none"!==q.getComputedStyle(e).display)return!1;XF.display(e)},step:(e,{delta:g})=>{e.style.opacity=g.toString()},finish:e=>{e.style.opacity=""}}),fadeTo:(f,d={})=>b(f,{...d,start:e=>{const g=Number(q.getComputedStyle(e).opacity);var h;const k=null!=(h=d.opacity)?h:1;if(g===
k)return!1;h=k-g;XF.display(e);return{initialOpacity:g,deltaOpacity:h}},step:(e,{delta:g,initialOpacity:h,deltaOpacity:k})=>{e.style.opacity=(h+k*g).toString()}}),slideUp:(f,d={})=>b(f,{...d,start:e=>{if("none"===q.getComputedStyle(e).display)return!1;const g=e.offsetHeight;e.style.overflow="hidden";return{initialHeight:g}},step:(e,{delta:g,initialHeight:h})=>{e.style.height=h*(1-g)+"px"},finish:e=>{XF.display(e,"none");e.style.height="";e.style.overflow=""}}),slideDown:(f,d={})=>b(f,{...d,start:e=>
{if("none"!==q.getComputedStyle(e).display)return!1;XF.display(e);const g=e.offsetHeight;e.style.overflow="hidden";return{targetHeight:g}},step:(e,{delta:g,targetHeight:h})=>{e.style.height=h*g+"px"},finish:e=>{e.style.height="";e.style.overflow=""}}),fadeUp:(f,d={})=>b(f,{...d,start:e=>{if("none"===q.getComputedStyle(e).display)return!1;const g=e.offsetHeight;e.style.overflow="hidden";return{initialHeight:g}},step:(e,{delta:g,initialHeight:h})=>{e.style.height=h*(1-g)+"px";e.style.opacity=(1-g).toString()},
finish:e=>{XF.display(e,"none");e.style.height="";e.style.opacity="";e.style.overflow=""}}),fadeDown:(f,d={})=>b(f,{...d,start:e=>{if("none"!==q.getComputedStyle(e).display)return!1;XF.display(e);const g=e.offsetHeight;e.style.overflow="hidden";return{targetHeight:g}},step:(e,{delta:g,targetHeight:h})=>{e.style.height=h*g+"px";e.style.opacity=g.toString()},finish:e=>{e.style.height="";e.style.opacity="";e.style.overflow=""}})}})();XF.Transition=(()=>{const a=d=>{if(!(d&&d instanceof Element))return 0;
...
t=>{t.more&&t.ids&&t.ids.length?(t.status&&(m.querySelector("#xfAutoBlockingJobStatus").textContent=t.status),setTimeout(()=>{k(t.ids)},0)):(l(),t.moreAuto&&setTimeout(h,100))},{skipDefault:!0});y.catch(l);e=r},l=()=>{g&&g.hide();d--;0>d&&(d=0);0===d&&(XF.trigger(m,"job:auto-blocking-complete"),x());e&&e.abort();e=null},p=()=>{f&&f.hide();c=[];a=!1;XF.trigger(m,"job:manual-complete");x();b&&b.abort();b=null},n=()=>{f||(f=u("xfManualJobStatus"));return f},u=v=>{v=XF.getOverlayHtml({title:XF.phrase("processing..."),
dismissible:!1,html:`<div class="blockMessage"><span id="${v}">${XF.phrase("processing...")}</span></div>`});return new XF.Overlay(v,{backdropClose:!1,keyboard:!1})},x=()=>{a||0<d||XF.trigger(m,"job:blocking-complete")};return{isBlockingJobRunning:()=>a||0<d,runAuto:h,runAutoBlocking:(v,y)=>{if("number"===typeof v)v=[v];else if(!Array.isArray(v))return;v.length&&(d++,g||(g=u("xfAutoBlockingJobStatus")),g.show(),y||(y=XF.phrase("processing...")),m.querySelector("#xfAutoBlockingJobStatus").textContent=
y,k(v))},runManual:v=>{const y=XF.config.job.manualUrl;if(y&&(null===v?c=null:(c=c||[],"number"===typeof v?c.push(v):Array.isArray(v)&&(c=[...c,...v])),!a)){a=!0;n().show();var r=w=>{const {ajax:z,controller:A}=XF.ajaxAbortable("post",y,w?{only_id:w}:null,B=>{B.jobRunner?(m.querySelector("#xfManualJobStatus").textContent=B.jobRunner.status||XF.phrase("processing..."),setTimeout(()=>{r(w)},0)):t()},{skipDefault:!0});z.catch(p);b=A},t=()=>{Array.isArray(c)&&0===c.length?p():r(c?c.shift():null)};t()}},
...
f.replace("__SENTINEL__",h.join(",")),fetch(f,{headers:{Accept:"text/css"}}).then(l=>{if(!l.ok)throw Error("Network response was not ok.");return l.text()}).then(l=>{const p=XF.config.url.basePath;p&&(l=l.replace(/(url\(("|')?)([^"')]+)(("|')?\))/gi,(n,u,x,v,y,r)=>{v.match(/^([a-z]+:|\/)/i)||(v=p+v);return u+v+y}));XF.createElement("style",{textContent:l},m.head);h.forEach(n=>{a[n]=!0});k--;0===k&&e&&e()})):(console.error("No CSS URL so cannot dynamically load CSS"),k--,0===k&&e&&e())))};return{load:b,
...
h):c.removeAttribute("data-color-scheme");d(l.metaThemeColor);var n=m.querySelectorAll("picture[data-variations]");for(var u of n){n=XF.createElement("picture");const t=JSON.parse(u.dataset.variations);var x=u.querySelector("img");x={properties:{},attributes:{width:x.getAttribute("width"),height:x.getAttribute("height"),alt:x.getAttribute("alt")}};if(""!==g)x.properties.src=t[g][1],t[g][2]&&(x.properties.srcset=`${t[g][2]} 2x`);else{var v=XF.config.style.defaultColorScheme;const w="dark"===v?"light":
"dark";v=XF.config.style[v];const z=XF.config.style[w];for(const [A,B]of Object.entries(t))if(A===v)x.properties.src=B[1],B[2]&&(x.properties.srcset=`${B[2]} 2x`);else if(A===z&&(B[1]!==t[v][1]||B[2]!==t[v][2])){const G=XF.createElement("source",{srcset:`${B[1]}${B[2]?`, ${B[2]} 2x`:""}`,media:`(prefers-color-scheme: ${w})`});n.prepend(G)}}n.append(XF.createElement("img",x));u.innerHTML="";u.append(...n.childNodes)}u=m.querySelectorAll(".js-styleVariationsMenu");for(var y of u)XF.trigger(y,"menu:close");
if(k){y=m.querySelectorAll(".js-styleVariationsLink");for(var r of y)if(y=r.querySelector("i.fa--xf"))u=XF.createElementFromString(XF.Icon.getIcon("default",k)),y.replaceWith(u)}r=m.querySelectorAll(".js-styleVariationsMenu");for(const t of r){r=t.querySelectorAll(".menu-linkRow");for(const w of r)w.dataset.variation===g?w.classList.add("is-selected"):w.classList.remove("is-selected")}e=XF.customEvent("xf:variation-change",{oldVariation:p,newVariation:g,data:e});XF.trigger(m,e)},d=e=>{if(e){var g=
m.querySelector("head");g.querySelectorAll("meta[name=theme-color]").forEach(h=>h.remove());if("object"!==typeof e)XF.createElement("meta",{name:"theme-color",content:e},g);else for(const h in e)XF.createElement("meta",{name:"theme-color",media:`(prefers-color-scheme: ${h})`,content:e[h]},g)}};return{getVariation:a,getColorScheme:()=>{const e=m.querySelector("html").dataset.colorScheme;if(e)return e;if(q.matchMedia("(prefers-color-scheme: dark)").matches&&XF.config.style.dark)return"dark";let g;return null!=
(g=XF.config.style.defaultColorScheme)?g:"light"},updateVariation:e=>{if(!b){b=!0;var g="admin"===m.querySelector("html").dataset.app?"admin.php?account/style-variation":"index.php?misc/style-variation";e=e?{variation:e}:{reset:1};XF.ajax("GET",XF.canonicalizeUrl(g),e,f,{skipDefault:!0})}}}})();XF.Icon=(()=>{const a=/<svg [^>]*viewBox="(?<viewBox>[^"]+)"[^>]*>.*?(?:<defs>.*<\/defs>)?(?<icon>(?:<path [^>]*\/>)+)<\/svg>/i,c=(h,k,l,p)=>{l=void 0!==l?l:"";l=`${d(h)} ${e(k)} ${l}`;return`<i class="fa--xf ${l}">${p}</i>`},
b=(h,k)=>{h=g(h);k=k.replace(/^fa-/,"");return XF.config.url.icon.replace("__VARIANT__",h).replace("__NAME__",k)},f=(h,k)=>{h=g(h);k=k.replace(/^fa-/,"");return XF.config.url.iconInline.replace("__VARIANT__",h).replace("__NAME__",k)},d=h=>{let k="far";h=g(h);switch(h){case "light":k="fal";break;case "regular":k="far";break;case "solid":k="fas";break;case "duotone":k="fad";break;case "brands":k="fab"}return k},e=h=>{h=h.replace(/^fa-/,"");return`fa-${h}`},g=h=>{let k="regular";"default"===h&&(h=XF.config.fontAwesomeWeight);
switch(h){case 300:case "l":case "fal":case "light":k="light";break;case 400:case "r":case "far":case "regular":k="regular";break;case 900:case "s":case "fas":case "solid":k="solid";break;case "d":case "fad":case "duotone":k="duotone";break;case "b":case "fab":case "brands":k="brands"}return k};return{ICON_DATA_REGEX:a,ICON_CLASS_REGEX:/^fa-(?!-)(?<name>[a-z0-9-]+)$/i,ICON_CLASS_BLOCKLIST_REGEX:/^fa-(xs|sm|lg|\d+x|fw|ul|li|rotate-\d+|flip-(horizontal|vertical|both)|spin|pulse|border|pull-(left|right)|stack(-\dx)?|inverse)$/i,
