...
k.querySelectorAll("[colspan], [rowspan]").forEach(p=>{p.removeAttribute("colspan");p.removeAttribute("rowspan")});let m=0;k.querySelectorAll(":scope > tbody > tr").forEach(p=>{p=p.querySelectorAll(":scope > td, :scope > th").length;m=Math.max(m,p)});k.querySelectorAll(":scope > tbody > tr").forEach(p=>{var q=p.querySelectorAll(":scope > td, :scope > th"),v=q.length;if(v<m)for(q=v&&"TH"===q[0].tagName?"th":"td";v<m;v++){const x=u.createElement(q);p.appendChild(x)}})});e.querySelectorAll("code, del, ins, sub, sup").forEach(k=>
{for(;k.firstChild;)k.parentNode.insertBefore(k.firstChild,k);k.remove()});let d=!1;e.querySelectorAll("h1").forEach(k=>{d=!0;const l=u.createElement("h2");l.appendChild(...k.childNodes);k.parentNode.replaceChild(l,k)});const c={H2:d?"H3":"H2",H3:d?"H4":"H3",H4:"H4",H5:"H4",H6:"H4"};e.querySelectorAll("h2, h3, h4, h5, h6").forEach(k=>{const l=u.createElement(c[k.tagName]);l.appendChild(...k.childNodes);k.parentNode.replaceChild(l,k)});if(a=e.querySelector("pre")){var f=a.innerHTML;f=f.replace(/\r?\n/g,
"<br>").replace(/\t/g,"    ").replace(/ {2}/g,"&nbsp; ").replace(/ {2}/g,"&nbsp; ").replace(/> /g,">&nbsp;").replace(/<br> /g,"<br>&nbsp;");var g=u.createElement("div");g.innerHTML=f+"<br>";a.parentNode.replaceChild(g,a)}b.opts.imagePaste||e.querySelectorAll("img[data-fr-image-pasted]").forEach(k=>{k.getAttribute("src").match(/https?:\/\//i)&&k.removeAttribute("data-fr-image-pasted")});e.querySelectorAll("br").forEach(k=>{for(var l=[],m=k.parentNode;m&&m!==e;)l.push(m),m=m.parentNode;if(0!==l.length&&
!l.some(x=>b.node.isBlock(x))){m=[];var p=!1,q=k;l=l[l.length-1];do{for(;q.nextSibling;){var v=q.nextSibling.cloneNode(!0);p?m.push(v):m=m.concat(v);q.parentNode.removeChild(q.nextSibling)}q=q.parentNode;if(!q||q===e)break;v=q.cloneNode();v.innerHTML="";m.forEach(x=>{v.appendChild(x)});m=[v];p=!0}while(q.parentNode&&q.parentNode!==e);k.parentNode.removeChild(k);l.after(...m);l.after(u.createElement("br"))}});a="";f=e.textContent.replace(/\s/g,"");try{a=(b.win.localStorage.getItem("fr-copied-text")||
"").replace(/\s/g,"")}catch(k){}a!==f&&e.querySelectorAll("p:not([data-xf-p])").forEach(k=>{if(k.nextSibling){const l=u.createElement("p");k.parentNode.insertBefore(l,k.nextSibling)}});e.querySelectorAll("p").forEach(k=>k.removeAttribute("data-xf-p"));f=Array.from(e.childNodes);a=u.createElement("div");g=null;for(var n of f)n.nodeType===Node.ELEMENT_NODE&&b.node.isBlock(n)?(a.appendChild(n),g=null):n.nodeType===Node.ELEMENT_NODE&&"BR"===n.tagName?(g||a.appendChild(u.createElement("p")),g=null):(g||
(g=u.createElement("p"),a.appendChild(g)),g.appendChild(n));n=a.children;1!==n.length||"P"!==n[0].tagName&&"DIV"!==n[0].tagName||(a=n[0]);return XF.EditorHelpers.normalizeBrForEditor(a.innerHTML)},watchEditorHeight(){const a=this.ed;XF.on(C,"resize",()=>{const d=this.getHeightLimits();a.opts.heightMin=d[0];a.opts.heightMax=d[1];a.size.refresh();XF.layoutChange()},{passive:!0});a.events.on("focus",()=>this.scrollToCursorAfterPendingResize());let b=a.$wp.height();const e=()=>{const d=a.$wp.height();
b!=d&&(b=d,XF.layoutChange())};a.events.on("keyup",e);a.events.on("commands.after",e);a.events.on("html.set",e);a.events.on("init",e);a.events.on("initialized",e)},getHeightLimits(){let a=this.options.maxHeight;const b=this.options.minHeight;var e=null;let d=null;this.target.closest(".overlay")&&(a=.1);a&&(e=u.documentElement.clientHeight,/(iPad|iPhone|iPod)/g.test(navigator.userAgent)&&(e-=250),e=Math.floor(0<a?1>=a?e*a:a:e+a),e=Math.max(e,150));b&&e&&(d=Math.min(b,e),d==e&&--d);return[d,e]},setupUploads(){const a=
this,b=this.ed;b.events.on("file.uploaded",function(d){this.popups.hide("file.insert");this.events.focus();return a.handleUploadSuccess(d)});b.events.on("file.error",function(d,c){this.popups.hide("file.insert");a.handleUploadError(d,c);this.events.focus();return!1});this.uploadUrl||(b.events.on("image.beforeUpload",function(){return!1}),b.events.on("file.beforeUpload",function(){return!1}),b.events.on("video.beforeUpload",function(){return!1}));b.events.on("image.error",function(d,c){if(c)return this.popups.hide("image.insert"),
a.handleUploadError(d,c),!1});b.events.on("video.error",function(d,c){if(c)return this.popups.hide("video.insert"),a.handleUploadError(d,c),!1});b.events.on("image.uploaded",function(d){return a.handleUploadSuccess(d,function(){b.image.remove();b.popups.hide("image.insert");b.events.focus();return!1},function(){return!0})});b.events.on("video.uploaded",function(d){return a.handleUploadSuccess(d,function(){b.video.remove();b.popups.hide("video.insert");b.events.focus();return!1},function(){return!0})});
const e=function(d,c){if(c){try{var f=JSON.parse(c)}catch(g){return}d.hasClass("fr-video")&&(d=d.find("video"),d.attr("data-xf-init","video-init").attr("style","").empty());if(f.attachment){f=f.attachment.attachment_id;c=d[0].attributes;const g=/^data-(?!xf-init)/;for(let n=c.length-1;0<=n;n--)g.test(c[n].nodeName)&&d.removeAttr(c[n].nodeName);d.attr("data-attachment","full:"+f)}}};b.events.on("image.inserted video.inserted",e);b.events.on("image.replaced video.replaced",e);b.events.on("image.loaded",
function(d){if(b.popups.isVisible("image.edit")){var c=b.image.get();c&&c[0]==d[0]&&(c.attr("data-size",`${c[0].naturalWidth}x${c[0].naturalHeight}`),b.image.exitEdit(!0),c=b.selection.ranges(0),c.setStartAfter(d[0]),c.collapse(!0),d=b.selection.get(),d.removeAllRanges(),d.addRange(c),b.events.focus(),a.scrollToCursor())}});b.events.on("video.loaded",function(d){if(b.popups.isVisible("video.edit")){var c=b.video.get();c&&c[0]==d[0]&&(b.events.trigger("video.hideResizer"),b.popups.hide("video.edit"),
c=b.selection.ranges(0),c.setStartAfter(d[0]),c.collapse(!0),d=b.selection.get(),d.removeAllRanges(),d.addRange(c),b.events.focus(),a.scrollToCursor())}});b.events.on("popups.show.image.edit",function(){const d=b.image.get();d.length&&d.hasClass("smilie")&&(b.image.exitEdit(!0),b.selection.save(),setTimeout(function(){b.selection.restore()},0))})},handleUploadSuccess(a,b,e){let d;try{d=JSON.parse(a)}catch(c){d={status:"error",errors:[XF.phrase("oops_we_ran_into_some_problems")]}}return d.status&&
"error"==d.status?(XF.alert(d.errors[0]),b?b(d):!1):(a=this.getAttachmentManager())&&d.attachment?(a.insertUploadedRow(d.attachment),e?e(d,a):!1):!1},handleUploadError(a,b){let e;try{e=JSON.parse(b)}catch(d){e=null}e&&e.errors||(e={status:"error",errors:[XF.phrase("oops_we_ran_into_some_problems")]});XF.alert(e.errors[0])},getAttachmentManager(){const a=this.target.closest("[data-xf-init~=attachment-manager]");return a?XF.Element.getHandler(a,"attachment-manager"):null},isBbCodeView(){return this.ed.bbCode&&
this.ed.bbCode.isBbCodeView?this.ed.bbCode.isBbCodeView():!1},insertContent(a,b){const e=this.ed;this.isBbCodeView()?"undefined"!==typeof b&&e.bbCode.insertBbCode(b):(this.focus(),e.undo.saveStep(),e.html.insert(a),e.undo.saveStep(),XF.Element.initialize(e.$el[0]),XF.EditorHelpers.normalizeAfterInsert(e));this.scrollToCursor();this.scrollToCursorAfterPendingResize()},replaceContent(a,b){const e=this.ed;this.isBbCodeView()?"undefined"!==typeof b&&e.bbCode.replaceBbCode(b):e.html.set(a)},scrollToCursor(){const a=
this.ed;if(this.isBbCodeView())XF.autofocus(a.bbCode.getTextArea()),a.$box[0].scrollIntoView(!0);else{this.focus();var b=a.$box;const d=a.$wp,c=a.selection.endElement();var e=c.getBoundingClientRect().bottom;let f=!0,g=XF.windowHeight();XF.browser.ios&&(g-=250);if(0>e||e>=g)f=!1;if(d&&f){const n=d[0].getBoundingClientRect();if(e>n.bottom||e<n.top)f=!1}if(!f){e=b[0].getBoundingClientRect();if(0>e.top||e.bottom>=g)XF.browser.ios||b.addClass("is-scrolling-to"),b[0].scrollIntoView(!0),b.removeClass("is-scrolling-to");
d?(b=a.position.getBoundingRect().top,b>d.offset().top-a.helpers.scrollTop()+d.height()-50&&d.scrollTop(b+d.scrollTop()-(d.height()+d.offset().top)+a.helpers.scrollTop()+g/2)):c.scrollIntoView()}}},scrollToCursorAfterPendingResize(a){const b=function(){c&&clearTimeout(c);c=setTimeout(g,100)},e=this,d=this.ed;let c;const f=function(){XF.off(C,"resize",f);XF.on(C,"scroll",b);c&&clearTimeout(c);c=setTimeout(g,500)},g=function(){XF.off(C,"scroll",b);d.core.hasFocus()&&e.scrollToCursor()};XF.on(C,"resize",
f);setTimeout(function(){XF.off(C,"resize",f)},2E3);a&&(c=setTimeout(g,1E3))},base64ToBytes(a,b){b=b||512;a=atob(a);const e=[];for(let c=0;c<a.length;c+=b){var d=a.slice(c,c+b);const f=Array(d.length);for(let g=0;g<d.length;g++)f[g]=d.charCodeAt(g);d=new Uint8Array(f);e.push(d)}return e},editorSupportsUploads(){return-1!==this.ed.opts.imageInsertButtons.indexOf("imageUpload")},imageMatchesBase64Encoding(a){return a.attr("src").match(/^data:(image\/([a-z0-9]+));base64,(.*)$/)},replaceBase64ImageWithUpload(a){if(!a.hasClass("smilie")){var b;
if(b=this.imageMatchesBase64Encoding(a)){var e=b[1];var d=b[2];b=b[3];-1===this.ed.opts.imageAllowedTypes.indexOf(d)?a[0].remove():this.editorSupportsUploads()?(a=new Blob(this.base64ToBytes(b),{type:e}),1024<a.size&&this.ed.image.upload([a])):a[0].remove()}}},isInitialized(){return this.ed?!0:!1}});XF.EditorButtons=XF.create({xfEd:null,buttonClasses:null,toolbars:{},dropdowns:{},removeButtons:null,recalculateNeeded:!0,__construct(a,b,e){this.xfEd=a;this.removeButtons=[];b&&(this.toolbars=b);e&&(this.dropdowns=
e)},addToolbar(a,b){this.toolbars[a]=b;this.recalculateNeeded=!0},adjustToolbar(a,b){const e=this.toolbars[a];return e?(this.toolbars[a]=b(e,a,this),this.recalculateNeeded=!0):!1},adjustToolbars(a){for(const b of Object.keys(this.toolbars))this.adjustToolbar(b,a)},getToolbar(a){return this.getToolbars()[a]},getToolbars(){this.recalculateIfNeeded();if(XF.EditorHelpers.isPreviewAvailable(this.xfEd.target))for(const a of Object.keys(this.toolbars))this.toolbars[a].preview={buttons:["xfPreview"],align:"right"};
return this.toolbars},addDropdown(a,b){this.dropdowns[a]=b;this.recalculateNeeded=!0},adjustDropdown(a,b){const e=this.dropdowns[a];return e?(this.dropdowns[a]=b(e,a,this),this.recalculateNeeded=!0):!1},adjustDropdowns(a){for(const b of Object.keys(this.dropdowns))this.adjustDropdown(b,a)},getDropdown(a){return this.getDropdowns()[a]},getDropdowns(){this.recalculateIfNeeded();return this.dropdowns},addRemovedButton(a){this.removeButtons.push(a);this.recalculateNeeded=!0},addRemovedButtons(a){for(const b of a)this.removeButtons.push(b);
...
"\n"),b=u.createRange().createContextualFragment(b).textContent.trim());e.selection.save();c?(c.getDoc().setValue(b),c.focus()):(c=a.querySelector(".js-codeEditor"),c.value=b,c.focus())},_init(a){XF.on(u.querySelector("#editor_code_form"),"submit",this.submit.bind(this))},submit(a){a.preventDefault();a=this.ed;const b=this.overlay;var e=b.container.querySelector(".CodeMirror");if(e){e=e.CodeMirror;var d=e.getDoc();e.save();d.setValue("");e.setOption("mode","")}e=u.querySelector("#editor_code_type");
...
{root:v,rootMargin:"0px 0px 100px 0px"});v.querySelectorAll(".js-gif img:not(.js-observed)").forEach(r=>{r.classList.add("js-observed");h.observe(r)});const t=new IntersectionObserver(f,{root:v,rootMargin:"0px 0px 50px 0px"});v.querySelectorAll(".js-gifLoadMore").forEach(r=>{t.observe(r)});v.querySelectorAll(".js-gif").forEach(r=>XF.on(r,"click",d))}function d(h){h=h.currentTarget.querySelector("img");const t=h.parentNode;if(!t.classList.contains("is-loading")){t.classList.add("is-loading");var r=
h.dataset.insert,w=u.createElement("img");w.setAttribute("src",r);w.classList.add("fr-fic","fr-dii","fr-draggable");w.alt=h.alt;h=function(){b.selection.restore();XF.EditorHelpers.focus(b);b.undo.saveStep();b.html.insert(w.outerHTML);b.undo.saveStep();b.selection.save();XF.EditorHelpers.blur(b);XF.EditorHelpers.normalizeAfterInsert(b);q&&q.querySelector(".js-gifCloser").click();t.classList.remove("is-loading")};if(w.complete)h();else XF.on(w,"load",h)}}function c(h,t){for(const r of h)if(h=r.target,
r.isIntersecting)k(h);else a:{if(!h.dataset.loaded)break a;t=h.getAttribute("data-src");const w=h.getAttribute("src");h.setAttribute("src",t);h.setAttribute("data-src",w);h.dataset.loaded=""}}function f(h,t){for(const r of h)r.isIntersecting&&(h=r.target,n(h),t.unobserve(r.target))}function g(h){let t=h;h instanceof Event&&(t=h.target);if(t.offsetParent){var r=t.getBoundingClientRect(),w=r.bottom+100;t.childNodes.forEach(z=>{if(z.nodeType===Node.ELEMENT_NODE){var y=z.getBoundingClientRect();if(!(y.bottom<
r.top)){if(y.top>w)return!1;z.querySelectorAll(".js-gif img").forEach(B=>{B.getBoundingClientRect().top<=w&&k(B)})}}})}}function n(h){h.dataset.loading||(h.dataset.loading="1",XF.ajax("GET",h.dataset.href,t=>{t.html&&XF.setupHtmlInsert(t.html,r=>{const w=(r.matches(".js-gifContainer")?r:r.querySelector(".js-gifContainer")).closest(".js-gifContainer");setTimeout(()=>{h.outerHTML=w.innerHTML;e()},100)})}))}function k(h){if(!h.dataset.loaded){var t=h.getAttribute("data-src"),r=h.getAttribute("src");
h.setAttribute("src",t);h.setAttribute("data-src",r);h.dataset.loaded="1"}}function l(){const h=this,t=q.querySelector(".js-gifFullRow"),r=q.querySelector(".js-gifSearchRow");clearTimeout(A);A=setTimeout(function(){const w=h.value;if(!w||2>w.length)XF.display(r,"none"),XF.display(t),g(t);else{var z=XF.canonicalizeUrl("index.php?editor/insert-gif/search");XF.ajax("GET",z,{q:w},y=>{y.html&&XF.setupHtmlInsert(y.html,B=>{XF.display(t,"none");r.innerHTML=B.outerHTML;XF.display(r);v.scroll({top:0});e()})})}},
300)}let m=!1,p=!1,q,v,x=0,A;return{showMenu:function(){b.selection.save();XF.EditorHelpers.blur(b);var h=b.$tb.find('.fr-command[data-cmd="xfInsertGif"]')[0];if(!m){m=!0;let t=u.querySelector(".js-xfEditorMenu").innerHTML.trim();t=Mustache.render(t,{href:XF.canonicalizeUrl("index.php?editor/insert-gif")});q=u.createRange().createContextualFragment(t).querySelector(".menu");q.classList.add("menu--gif");h.insertAdjacentElement("afterend",q);h.dataset.xfClick="menu";const r=XF.Event.getElementHandler(h,
"menu","click");XF.on(h,"menu:complete",()=>{v=q.querySelector(".menu-scroller");if(!p){p=!0;e();const w=q.querySelector(".js-gifSearch");XF.on(w,"input",l);XF.on(q.querySelector(".js-gifCloser"),"click",()=>XF.EditorHelpers.focus(b));b.events.on("commands.mousedown",z=>{"xfInsertGif"!=z.data("cmd")&&r.close()});XF.on(q,"menu:closed",()=>{x=v.scrollTop})}v.scroll({top:x})});XF.on(q,"menu:closed",()=>{setTimeout(()=>{b.markers.remove()},50)})}(h=XF.DataStore.get(h,"xf-click-handlers"))&&h.menu&&h.menu.toggle()}}};
FroalaEditor.PLUGINS.xfSmilie=function(b){function e(h){const t=h.currentTarget,r=t.innerHTML;if(!t.classList.contains("smilie--lazyLoad")){XF.EditorHelpers.bypassBrowserShims(b,()=>{b.selection.restore();XF.EditorHelpers.focus(b);b.undo.saveStep();b.html.insert(r);b.undo.saveStep();b.selection.save();XF.EditorHelpers.blur(b);XF.EditorHelpers.normalizeAfterInsert(b)});if(m){const w=m.querySelector(".js-emojiInsertedRow");w.querySelector(".js-emojiInsert").innerHTML=r;XF.Transition.addClassTransitioned(w,
"is-active");clearTimeout(v);v=setTimeout(()=>XF.Transition.removeClassTransitioned(w,"is-active"),1500)}clearTimeout(x);x=setTimeout(()=>XF.logRecentEmojiUsage(t.dataset.shortname),1500)}}function d(h,t){for(const r of h)r.isIntersecting&&(h=r.target,f(h),t.unobserve(r.target))}function c(h,t){let r=h;h instanceof Event&&(r=h.target);if(t||r.offsetParent){var w=r.getBoundingClientRect(),z=w.bottom+100;r.childNodes.forEach(y=>{if(y.nodeType===Node.ELEMENT_NODE){var B=y.getBoundingClientRect();if(!(B.bottom<
w.top)){if(B.top>z)return!1;y.querySelectorAll("span.smilie--lazyLoad").forEach(D=>{D.getBoundingClientRect().top<=z&&f(D)})}}})}}function f(h){const t=XF.createElement("img",{className:h.getAttribute("class").replace(/(\s|^)smilie--lazyLoad(\s|$)/," "),alt:h.getAttribute("data-alt"),title:h.getAttribute("title"),src:h.getAttribute("data-src"),dataset:{shortname:h.dataset.shortname}}),r=()=>{C.requestAnimationFrame(()=>{h.outerHTML=t.outerHTML})};if(t.complete)XF.on(t,"load",r);else r()}function g(){const h=
this,t=m.querySelector(".js-emojiFullList"),r=m.querySelector(".js-emojiSearchResults");clearTimeout(A);A=setTimeout(()=>{const w=h.value;if(!w||2>w.length)XF.display(r,"none"),XF.display(t),c(t);else{var z=XF.canonicalizeUrl("index.php?editor/smilies-emoji/search");XF.ajax("GET",z,{q:w},y=>{y.html&&XF.setupHtmlInsert(y.html,B=>{XF.display(t,"none");r.innerHTML=B.outerHTML;XF.display(r);r.querySelectorAll(".js-emoji").forEach(D=>{XF.on(D,"click",e)})})})}},300)}function n(){let h;const t=XF.getRecentEmojiUsage(),
r=p.querySelector(".js-recentHeader"),w=p.querySelector(".js-recentBlock"),z=w.querySelector(".js-recentList"),y=p.querySelectorAll(".js-emojiList");if(t){var B=z.cloneNode(!0),D=[];B.innerHTML="";for(h in t){const F=t[h];y.forEach(E=>{if(E=E.querySelector(`.js-emoji[data-shortname="${F}"]`))return E=E.closest("li").cloneNode(!0),D.push(E),!1})}for(h in D)B.appendChild(D[h]);z.innerHTML=B.innerHTML;z.querySelectorAll(".js-emoji").forEach(F=>{XF.on(F,"click",e)});w.classList.contains("is-hidden")&&
(XF.display(w,"none"),w.classList.remove("is-hidden"),r.classList.remove("is-hidden"),XF.Animate.fadeDown(w,{speed:XF.config.speed.fast}));c(z,!0)}}let k=!1,l=!1,m,p,q=0,v,x,A;return{showMenu:function(){b.selection.save();XF.EditorHelpers.blur(b);var h=b.$tb.find('.fr-command[data-cmd="xfSmilie"]')[0];if(!k){k=!0;let t=u.querySelector(".js-xfEditorMenu").innerHTML.trim();t=Mustache.render(t,{href:XF.canonicalizeUrl("index.php?editor/smilies-emoji")});m=u.createRange().createContextualFragment(t).querySelector(".menu");
m.classList.add("menu--emoji");h.insertAdjacentElement("afterend",m);h.dataset.xfClick="menu";const r=XF.Event.getElementHandler(h,"menu","click");XF.on(h,"menu:complete",()=>{p=m.querySelector(".menu-scroller");if(!l){l=!0;const w=new IntersectionObserver(d,{root:p,rootMargin:"0px 0px 100px 0px"});p.querySelectorAll("span.smilie--lazyLoad").forEach(y=>{w.observe(y)});p.querySelectorAll(".js-emoji").forEach(y=>XF.on(y,"click",e));const z=m.querySelector(".js-emojiSearch");XF.on(z,"input",g);XF.on(m.querySelector(".js-emojiCloser"),
...
!l.isSameNode(k)&&!l.getAttribute("data-cmd").startsWith("more")&&"xfPreview"!==l.getAttribute("data-cmd"))}function f(k,l){const m=d(),p=function(q,v){n=!0;let x;b.undo.saveStep();b.$el.blur();x=e();c(x).forEach(A=>A.classList.add("fr-disabled"));x.classList.add("fr-active");b.$wp.css("display","none");b.$oel.attr("disabled","disabled");m.value=q;XF.display(m);m.disabled=!1;XF.trigger(m,"autosize");v||m.focus();XF.setIsEditorEnabled(!1)};"string"==typeof k?p(k,l):XF.ajax("POST",XF.canonicalizeUrl("index.php?editor/to-bb-code"),
{html:b.html.get()},function(q){p(q.bbCode,l)})}function g(k){const l=d(),m=function(p){n=!1;const q=e();c(q).forEach(v=>v.classList.remove("fr-disabled"));q.classList.remove("fr-active");b.$oel.removeAttr("disabled");b.html.set(p);XF.display(l,"none");l.disabled=!0;b.$wp.css("display","");b.events.focus();b.undo.saveStep();b.size.refresh();XF.setIsEditorEnabled(!0);XF.layoutChange()};if("string"==typeof k)m(k);else{k={bb_code:l.value};const p=b.$el[0].closest("form");p&&p[b.opts.xfBbCodeAttachmentContextInput]&&
...
b.$box.addClass("is-preview");b.bbCode.isBbCodeView()?(A=b.bbCode.getBbCodeBox(),XF.display(A,"none")):b.$wp.css("display","none");p.innerHTML="";p.append(...x.querySelector(".bbWrapper").childNodes);p.querySelectorAll("button:not([type])").forEach(h=>{h.type="button"});XF.display(p)};if("string"==typeof m)m=u.createRange().createContextualFragment(m),q(m.firstChild);else{b.events.trigger("form.submit");var v=b.$oel.closest("form")[0];if(m=b.$oel.data("preview-url")?b.$oel.data("preview-url"):v?v.dataset.previewUrl:
null)v=v?XF.getDefaultFormData(v):new FormData,XF.ajax("POST",XF.canonicalizeUrl(m),v,x=>{XF.setupHtmlInsert(x.html,A=>{XF.activate(A);q(A)})})}}function g(m){var p=d();const q=e(),v=b.bbCode.isBbCodeView();l=!1;c(q).forEach(x=>x.classList.remove("fr-disabled","fr-invisible"));q&&q.classList.remove("fr-active");v&&b.bbCode.getToggleableButtons().forEach(x=>x.classList.add("fr-disabled"));b.$tb.find(".fr-btn-grp").removeClass("rte-tab--inactive").filter(".rte-tab--preview").addClass("rte-tab--inactive");
b.$oel.removeAttr("disabled");XF.display(p,"none");b.$box.removeClass("is-preview");v?(p=b.bbCode.getBbCodeBox(),XF.display(p)):b.$wp.css("display","");m||b.events.focus();XF.layoutChange()}function n(){const m=b.$oel[0];XF.EditorHelpers.isPreviewAvailable(m)&&(l?g():(XF.EditorHelpers.sync(b),(b.bbCode&&b.bbCode.isBbCodeView()?b.bbCode.getBbCodeBox().value:m.value)&&f()))}function k(){const m=b.$tb.find(".fr-btn-grp");XF.EditorHelpers.isPreviewAvailable(b.$oel[0])?(m.slice(m.length-1).addClass("rte-tab--inactive rte-tab--preview"),
