...
this.timer=setTimeout(()=>this.draftHandler.triggerSave(),this.options.delay)}});XF.FocusTrigger=XF.Element.newHandler({options:{display:null,activeClass:"is-active"},init(){if(this.target.hasAttribute("autofocus"))this.trigger();else XF.on(this.target,"focusin",this.trigger.bind(this),{once:!0})},trigger(){var a=this.options.display;a&&(a=XF.findRelativeIf(a,this.target))&&XF.Transition.addClassTransitioned(a,this.options.activeClass)}});XF.PollBlock=XF.Element.newHandler({options:{},init(){XF.on(this.target,
