...
(a.preventDefault(),this.dragStart(c?a.touches[0]:a))}},onMoveEvent(a){const c="mousemove"!=a.type;this.dragEl&&(a.preventDefault(),this.dragMove(c?a.touches[0]:a))},onEndEvent(a){const c="mouseup"!=a.type;this.dragEl&&(a.preventDefault(),this.dragStop(c?a.touches[0]:a))},hasTouchEvents(){return XF.Feature.has("touchevents")},serialize(){const a=(c,b)=>{const d=[];Array.from(c.children).filter(e=>e.matches(this.options.itemNodeName)).forEach(e=>{const f={...e.dataset};e=Array.from(e.children).filter(g=>
g.matches(this.options.listNodeName));e.length&&(f.children=a(e,b+1));d.push(f)});return d};return a(this.target.querySelector(this.options.listNodeName),0)},asNestedSet(){let a=[],c=1;const b=(e,f,g)=>{let k=g+1;const q=this.options.listNodeName,r=this.options.itemNodeName;var l=e.querySelector(q);let n=[];l&&(n=Array.from(l.children).filter(p=>p.matches(`${r}`)));0<n.length&&(f++,n.forEach(p=>{k=b(p,f,k)}),f--);l=e.dataset.id;e=(e=e.closest(`${q}`).closest(`${r}`))?e.dataset.id:this.options.parentID;
l&&a.push({id:l,parent_id:e,depth:f,lft:g,rgt:k});return g=k+1},d=this.target.querySelector(this.options.listNodeName);d&&Array.from(d.children).forEach(e=>{e.nodeName.toLowerCase()===this.options.itemNodeName&&(c=b(e,0,c))});return a=a.sort((e,f)=>e.lft-f.lft)},reset(){this.mouse={offsetX:0,offsetY:0,startX:0,startY:0,lastX:0,lastY:0,nowX:0,nowY:0,distX:0,distY:0,dirAx:0,dirX:0,dirY:0,lastDirX:0,lastDirY:0,distAxX:0,distAxY:0,moving:!1};this.dragRootEl=this.dragEl=null;this.dragDepth=0;this.hasNewRoot=
...
this.options.collapseBtnHTML));a.querySelectorAll('[data-action="expand"]').forEach(c=>XF.display(c,"none"))},unsetParent(a){a&&(a.classList.remove(this.options.collapsedClass),a.querySelectorAll("[data-action]").forEach(c=>c.remove()),a.querySelectorAll(this.options.listNodeName).forEach(c=>c.remove()))},dragStart(a){var c=a.target,b=c.closest(this.options.itemNodeName);this.placeEl.style.height=`${b.offsetHeight}px`;var d=void 0!==a.offsetX?a.offsetX:a.pageX-c.getBoundingClientRect().left;c=void 0!==
a.offsetY?a.offsetY:a.pageY-c.getBoundingClientRect().top;this.mouse={offsetX:d,offsetY:c,startX:a.pageX,lastX:a.pageX,startY:a.pageY,lastY:a.pageY};this.dragRootEl=this.target;d=b.offsetWidth;this.dragEl=XF.createElement(this.options.listNodeName,{className:`${this.options.listClass} ${this.options.dragClass}`,style:{width:`${d}px`},dataset:{dragWidth:d}});b.insertAdjacentElement("afterend",this.placeEl);b.parentNode.removeChild(b);this.dragEl.append(b);m.body.append(this.dragEl);b=a.pageX-this.mouse.offsetX;
a=a.pageY-this.mouse.offsetY;XF.isRtl()&&(b-=this.dragEl.dataset.dragWidth);this.dragEl.style.left=`${b}px`;this.dragEl.style.top=`${a}px`;a=this.dragEl.querySelectorAll(this.options.itemNodeName);this.dragDepth=Array.from(a).reduce((e,f)=>{f=Array.from(f.querySelectorAll(this.options.listNodeName)).length;return f>e?f:e},0)},dragStop(a){a=this.dragEl.querySelector(this.options.itemNodeName);a.parentNode.removeChild(a);this.placeEl.replaceWith(a);this.dragEl.remove();XF.trigger(this.target,"change");
this.hasNewRoot&&XF.trigger(this.dragRootEl,"change");this.reset()},dragMove(a){const c=this.options;var b=this.mouse;var d=a.pageX-b.offsetX;var e=a.pageY-b.offsetY;XF.isRtl()&&(d-=this.dragEl.dataset.dragWidth);this.dragEl.style.left=`${d}px`;this.dragEl.style.top=`${e}px`;b.lastX=b.nowX;b.lastY=b.nowY;b.nowX=a.pageX;b.nowY=a.pageY;b.distX=b.nowX-b.lastX;b.distY=b.nowY-b.lastY;b.lastDirX=b.dirX;b.lastDirY=b.dirY;b.dirX=0===b.distX?0:0<b.distX?1:-1;b.dirY=0===b.distY?0:0<b.distY?1:-1;d=Math.abs(b.distX)>
Math.abs(b.distY)?1:0;if(b.moving){b.dirAx!==d?(b.distAxX=0,b.distAxY=0):(b.distAxX+=Math.abs(b.distX),0!==b.dirX&&b.dirX!==b.lastDirX&&(b.distAxX=0),b.distAxY+=Math.abs(b.distY),0!==b.dirY&&b.dirY!==b.lastDirY&&(b.distAxY=0));b.dirAx=d;if(b.dirAx&&b.distAxX>=c.threshold){b.distAxX=0;d=this.placeEl.previousElementSibling;if((XF.isRtl()?0>b.distX:0<b.distX)&&d&&!d.classList.contains(c.collapsedClass)){var f=Array.from(d.children).find(k=>k.tagName.toLowerCase()===c.listNodeName);e=0;let g=this.placeEl.parentNode;
for(;g;)g instanceof Element&&g.tagName.toLowerCase()!==c.listNodeName||e++,g=g.parentNode;e+this.dragDepth<=c.maxDepth&&(f?(f=d.querySelectorAll(c.listNodeName+":last-child")[0],f.appendChild(this.placeEl)):(f=XF.createElement(c.listNodeName,{className:c.listClass},d),f.appendChild(this.placeEl),this.setParent(d)))}if(XF.isRtl()?0<b.distX:0>b.distX)d=this.placeEl.nextElementSibling,d&&d.tagName.toLowerCase()===c.itemNodeName||(d=this.placeEl.parentNode,(e=this.closest(this.placeEl,c.itemNodeName))&&
e.after(this.placeEl),d.hasChildNodes()||this.unsetParent(d.parentNode))}f=!1;(this.pointEl=m.elementFromPoint(a.pageX-m.body.scrollLeft,a.pageY-(h.scrollY||m.documentElement.scrollTop)))&&this.pointEl.classList.contains(c.handleClass)&&(this.pointEl=this.pointEl.closest(c.itemNodeName));if(this.pointEl&&this.pointEl.classList.contains(c.emptyClass))f=!0;else if(!this.pointEl||!this.pointEl.classList.contains(c.itemClass))return;d=this.pointEl.closest("."+c.rootClass);e=this.dragRootEl.dataset.nestableId!==
d.dataset.nestableId;b.dirAx&&!e&&!f||e&&c.group!==parseInt(d.dataset.nestableGroup)||(b=this.dragDepth-1+this.countParents(this.pointEl,c.listNodeName),b>c.maxDepth||(b=this.pointEl.getBoundingClientRect(),b=a.clientY<b.top+b.height/2,a=this.placeEl.parentNode,f?(f=XF.createElement(c.listNodeName,{className:c.listClass}),f.appendChild(this.placeEl),this.pointEl.replaceWith(f)):b?this.pointEl.parentNode.insertBefore(this.placeEl,this.pointEl):this.pointEl.parentNode.insertBefore(this.placeEl,this.pointEl.nextElementSibling),
Array.from(a.children).length||this.unsetParent(a.parentNode),this.dragRootEl.querySelector(c.itemNodeName)||this.dragRootEl.querySelector("."+c.emptyClass)||this.appendEmptyElement(this.dragRootEl),this.dragRootEl=d,e&&(this.hasNewRoot=this.target!==this.dragRootEl)))}else b.dirAx=d,b.moving=!0},closest(a,c){for(;a;){if(a.matches(c))return a;a=a.parentElement}return null},countParents(a,c){let b=0;for(;a.parentNode;)a=a.parentNode,a.nodeName.toLowerCase()===c&&b++;return b},appendEmptyElement(a){a.append(XF.createElementFromString(`<div class="${this.options.emptyClass}"></div>`))}});
