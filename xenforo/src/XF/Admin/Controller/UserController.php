...
class UserController extends AbstractController
{
	...
	protected function userSaveProcess(\XF\Entity\User $user)
	{
		...
		if ($user->user_id && isset($input['user']['secondary_group_ids']))
		{
			// preserve change-managed groups (moderator/promotion/upgrade/ban) that
			// are not present in the submitted checkbox state, matching the API path
			$input['user']['secondary_group_ids'] = $this->service(UserGroupChangeService::class)
				->mergeSecondaryGroupIds(
					$user->user_id,
					$input['user']['secondary_group_ids']
				);
		}
